...
function SetImg(id, name, src, list)
{
...
function SetImg3(id, name, src, list)
{
	o = document.getElementById(id);
	//o.onclick =  "javascript:viewPic(this)";
	o.width = 60;
...
function getJsonArray(arr, name)
{
	var i = offset;
    if ( arr !== undefined && arr !== null )
    {
		for(let idx in arr) {
			if (arr[idx].ListName == name)
			{
				return arr[idx].ListData;
			}
		}
		return arr[0].ListData;
	}
	return null;
}

function loadArray2(arr, offset)
{
	var i = offset;
    if ( arr !== undefined && arr !== null )
    {
		for(let idx in arr.ListData) {
			SetImg2(i++, arr.ListData[idx]);
			//SetImg(i++, arr.ListData[idx].title, arr.ListData[idx].src);
		}
	}
}
function loadArray(arr, offset)
{
	var i = offset;
    if ( arr !== undefined && arr !== null )
    {
...
//	}
	...
	function changesourcerelative2(offset) {
			...
			var idx = getnameidx3(decodeURI(name), arr);
	...
	function changesourceN(idx, source, arr) {
	    ...
	    document.getElementById("antal").innerHTML = picnr(adjustidx( idx, arr.length) + 1, arr.length);
	    document.getElementById("info").innerHTML =  "";
	    document.getElementById("info2").innerHTML = fld;
	}
